...
function untilNext(min, hour) {   
    const nextLod = channels.findIndex((_, row) => row * 3 > hour) * 3;
    const hoursUntilNext = nextLod - hour - 1;
    const minsUntilNext = 60 - min;
    return `Quedan ${hoursUntilNext} horas y ${minsUntilNext} minutos para el próximo LoD.`;
}
...
function createLodEmbed() {
	...
	.addFields(
		{ name: '\u200B', value: '\u200B' },
		{ name: 'Actual', value: hoursPassed < 2 ? `Hay LoD ahora mismo en ${getChannels("current")}. ${currentLod(min, hour)}` : `(El próximo LoD es a las ${nextLod}:00 en ${getChannels("next")}. ${untilNext(min, hour)}`, inline: true },
